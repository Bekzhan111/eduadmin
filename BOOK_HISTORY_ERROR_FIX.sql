-- Direct SQL fix for book edit history error
-- Run this in the Supabase SQL Editor to fix the issue

-- Update existing history trigger function with proper security context
...
-- Update snapshot saving function
...
BEGIN
    ...
    IF history IS NULL THEN
        ...
        new_entry := jsonb_build_object(
            ...
            'user_id', current_user_id,
            'user_name', COALESCE(user_name, 'Unknown User'),
            'elements', COALESCE(canvas_data, '[]'::JSONB),
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'books' AND column_name = 'edit_history'
    ) THEN
        ALTER TABLE books ADD COLUMN edit_history JSONB DEFAULT NULL;
    END IF;
END
$$;

-- Recreate the trigger (will only be applied to future updates)
