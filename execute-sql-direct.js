...
async function executeSQLDirect() {
  console.log('🔧 Выполнение SQL обновления через REST API\n');
...
`;
  ...
  console.log('📝 Попытка выполнения SQL через REST API...');
  
  // Извлекаем URL и ключ из переменных окружения
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceKey = process.env.NEXT_PUBLIC_SERVICE_ROLE_KEY;
  
  if (!supabaseUrl || !serviceKey) {
    console.error('❌ Отсутствуют переменные окружения SUPABASE_URL или SERVICE_ROLE_KEY');
    return;
  }
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          console.log('✅ SQL скрипт выполнен успешно через REST API');
          resolve(data);
        } else {
          console.error(`❌ Ошибка HTTP ${res.statusCode}:`, data);
          console.log('\n⚠️ REST API не поддерживает выполнение произвольного SQL');
          ...
          console.log('4. Выполните содержимое файла update-dashboard-function.sql');
          console.log('\n🎯 После выполнения SQL скрипта, автор увидит "Панель Автора"');
          console.log('📧 Тестовые данные: test-author-fixed@example.com / TestPassword123!');
